"""Build the attack_deputy submission notebook: embeds the current attack_deputy.py (base64)
into the shared notebook template (tools/notebook_template.ipynb + kernel-metadata.template.json).

Usage:  python tools/build_deputy_nb.py
Outputs submission_deputy/ (regenerated fresh each run; not kept in git -- see STATUS.md's
file-map note on the 2026-09-02 cleanup).
"""
...
ATTACK_SRC = ROOT / "src" / "attack_deputy.py"
...
def build() -> Path:
    ...
    nb["cells"][0]["source"] = [
        "# AI Agent Security - Attack (attack_deputy: CONFUSED_DEPUTY universal-survivor hedge)"
    ]
    ...
    out_dir = ROOT / "submission_deputy"
    ...
    assert "CONFUSED_DEPUTY" in code and "email.send" in code
    print(f"built {out_dir.name}: {len(code)} bytes, compiles, deputy mechanism present")
    return out_dir
